...
class _ClientOrdersPageState extends State<ClientOrdersPage> {
  ...
  Future<void> _resubmitCancelledOrder(ClientOrder order) async {
    try {
      ...
      // Keep all prior request data for resubmit, but force Need By and JNT
      // Reveal Date to be empty -- those two are date-specific to the prior
      // submission and must be re-chosen, everything else should carry over.
      requestDetails.remove('needBy');
      requestDetails.remove('needByDisplay');
      requestDetails.remove('jntRevealDate');
      requestDetails.remove('jntRevealDateDisplay');
      initialRequestData.remove('needBy');
      initialRequestData.remove('needByDisplay');
      initialRequestData.remove('jntRevealDate');
      initialRequestData.remove('jntRevealDateDisplay');
